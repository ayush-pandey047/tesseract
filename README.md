...
This package includes an Optical Character Recognition (OCR) engine—libtesseract—and a command-line program called tesseract.

<h1>Tesseract 4 Features</h1>
Tesseract 4 introduces a new OCR (https://en.wikipedia.org/wiki/Optical_character_recognition) engine based on Long Short-Term Memory (LSTM) neural networks, which enhances line recognition. It also maintains compatibility with the legacy Tesseract 3 engine, which relies on character pattern recognition. To use the legacy engine, you can enable Legacy OCR Engine mode by specifying --oem 0. Note that this mode requires [trained](https://tesseract-ocr.github.io/tessdoc/Data-Files.html)data files compatible with the legacy engine, available from the [tessdata](https://github.com/tesseract-ocr/tessdata) repository.
